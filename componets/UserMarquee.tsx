    ...
    <div className="relative flex overflow-hidden whitespace-nowrap py-4 border-y border-white/5 bg-white/[0.02]">
      ...
      >
        {duplicatedItems.map((item, i) => (
          <span key={i} className={`flex items-center gap-2 font-black italic uppercase tracking-widest ${reverse ? 'text-white/[0.05] text-4xl' : 'text-white/30 text-xs'}`}>
            {!reverse && <span className="text-[#FF6154]">@</span>}
            {reverse ? "2025 RECAP" : item}
            {!reverse && <span className="mx-4 h-1 w-1 bg-white/20 rounded-full" />}
          </span>
        ))}
      </motion.div>
    </div>
